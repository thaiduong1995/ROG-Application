...
//    }
    ...
    fun providerListAllVideo() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            context.contentResolver.query(uriExternal, projection, null, null, null)?.let { cursor ->
                val columnIndexID: Int = cursor.getColumnIndexOrThrow(MediaStore.Video.Media._ID)
                while (cursor.moveToNext()) {
                ...
                cursor.close()
                _listVideoFlow.value = VideoState.Success(listOfAllVideo)
            }
        }
    }
}